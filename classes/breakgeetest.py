...
def solvegee(usethis, confirmgee):
    ...
    while Commands.IsSomething('confirmgee').do_work():
        ...
        try:
            ...
            print(math.ceil(usethis[2][0]), math.ceil(
                confirmgee[3][0]), (math.ceil(usethis[3][1])), math.ceil(confirmgee[2][1]))
            dst = src[(math.ceil(usethis[2][1])):math.ceil(
                confirmgee[2][1]), math.ceil(usethis[2][0]):math.ceil(confirmgee[3][0])]
            ...
            cv2.imwrite('geeres/main_pane'+str(time.time())+'.png', main_pane)
            cv2.imwrite('geeres/target'+str(time.time())+'.png',
                        img_grey[:60, math.ceil(usethis[3][0])-math.ceil(usethis[2][0]):])

            # Image preprocessing

            color_threshold = 180
            # By blurring, we can remove some white pixels which may affecting the matching
            main_pane = cv2.blur(main_pane, (3, 3))
            ...
            icons_rect_coordinates = find_bounding_box(
                main_pane, (20, 20), (100, 100), sort=False)
            print('finish bounding')
            icons = segment_pictures(
                main_pane, icons_rect_coordinates, (30, 30))
            draw_bounding_box(main_pane, icons_rect_coordinates)
            target_color_threshold = 40
            print('start target')
            target_pane = 255 - \
                img_grey[:60, math.ceil(usethis[3][0]) -
                         math.ceil(usethis[2][0]):]
            ...
            targets_rect_coordinates = find_bounding_box(
                target_pane, (5, 5), (100, 100))
            targets = segment_pictures(
                target_pane, targets_rect_coordinates, (30, 30))
            ...
            for target in targets:
                ...
                for icon in icons:
                    similarity_per_target.append(
                        calculate_max_matching(target, icon, 6))
            ...
            for key in mapping:
                ...
                centre_y = y+(h//2)
                Clicker.click(
                    (math.ceil(usethis[2][0])+centre_x, math.ceil(usethis[2][1])+centre_y+60))

                # Plot circle
                circle = plt.Circle(
                    (centre_x, centre_y), 20, color=color_map[key+1], fill=False, linewidth=5)
                # Plot centre
            ...
            pass
        if not Commands.IsSomething('confirmgee').do_work() or count >=20:
            break
